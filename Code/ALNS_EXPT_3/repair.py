...
from model_check import trip_split,objective_function,objective_function_fast,route_check_single,route_check_trips,cust_no_index,\
    find_time_slack_new,check_insertion_linear_new
...
def best_insertion(routes,data,initial_data,parameters):
    ...
    while len(unserved_cust) > 0:
        ...
        # print(sorted_list)
        INSERTED = False
        time2,time_slack2,time_adjustment_list2,demand_slack2 = find_time_slack_new(routes_trips,data,initial_data,parameters)
        for a in sorted_list:
            ...
            if check_insertion_linear_new(time2,time_slack2,time_adjustment_list2,demand_slack2,route,trip,k,r,i,cust,data,initial_data,parameters):
                # trip.remove(cust)
                # if a[1] < 0:
...
def best_insertion_fastest(routes,data,initial_data,parameters):
    ...
    while len(unserved_cust) > 0:
        ...
        for a in sorted_list:
            ...
            # proper_check = route_check_trips(route, data, initial_data, parameters,k,log= False)
            # trip.remove(cust)
            # assert proper_check ==  check_insertion_linear_new(time2,time_slack2,time_adjustment_list2,demand_slack2,route,trip,k,r,i,cust,data,initial_data,parameters,log = False)
